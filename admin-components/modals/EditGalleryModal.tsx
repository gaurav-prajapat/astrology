...
export default function EditGalleryModal({ image, onClose, onSuccess, onError }: EditGalleryModalProps) {
  const { t } = useLanguage();
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [formData, setFormData] = useState({
    title_en: image.title_en,
    title_hi: image.title_hi,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onError(error.message || 'Failed to update gallery image');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="p-6 flex justify-between items-center mb-0">
          <h3 className="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
            {t('Edit Gallery Image', 'गैलरी छवि संपादित करें')}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-grow px-6 pb-2">
          ...
          <div className="flex items-center space-x-3 px-4 py-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border-2 border-indigo-200 dark:border-indigo-800">
            ...
            />
            <label htmlFor="edit-gallery-active" className="text-gray-700 dark:text-gray-300 font-medium cursor-pointer">
        ...
        <div className="p-6 pt-0">
          ...
          >
            {loading ? t('Updating...', 'अपडेट कर रहे हैं...') : t('Update Image', 'छवि अपडेट करें')}
          </motion.button>
        </div>
      </motion.div>
    </div>
